...
$(document).ready(function () {
   ...
   function updateActiveDot(page) {
      ...
      decreaseInactiveDots(page);
   }

   function toggleNavigationButtons(page) {
      var totalPages = $("#flipbook").turn("pages");
      if (page === 1) {
         $("#prev-page").hide();
         $("#next-page").show();
   ...
   function decreaseInactiveDots(activePage) {
      $(".dot").each(function (index) {
         var pageNumber = parseInt($(this).attr("data-page"));
         var icon = (pageNumber === activePage) ? dotIcons[index] + " active" : dotIcons[index];
         $(this).attr("class", "dot " + icon);
      });
   }
   ...
   for (var i = 1; i <= totalPages; i++) {
      dotsContainer.append('<span class="dot" id="dot-' + i + '" data-page="' + i + '"></span>');
   }
   ...
   var scrollAmount = 60; // Adjust this value as needed
   var $dotsContainer = $("#dots-container");
   var totalDotsHeight = $dotsContainer[0].scrollHeight; // Total height of the dots
   var containerHeight = $dotsContainer.height();

   // Function to update button visibility
   function updateButtonVisibility() {
       var currentScroll = $dotsContainer.scrollTop();
       if (currentScroll <= 0) {
           $("#scroll-up-btn").css("opacity", 0);
           $(".flip-dots #dots-container").css("margin-top", "0");
       } else {
           $("#scroll-up-btn").css("opacity", 1);
           $(".flip-dots #dots-container").css("padding-top", "3rem");
       }

       if (currentScroll + containerHeight >= totalDotsHeight) {
